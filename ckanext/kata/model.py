...
def setup():
    ...
    '''
    if model.package_table.exists() and not _kata_access_request_table.exists():
        _kata_access_request_table.create()
        log.debug('Kata access request table created')

    if model.user_table.exists() and not _user_extra_table.exists():
        _user_extra_table.create()
...
def delete_tables():
    ...
    if _kata_access_request_table.exists():
        _kata_access_request_table.delete()


_kata_access_request_table = Table('kata_req', meta.metadata,
...
mapper(KataAccessRequest, _kata_access_request_table, extension=[
    extension.PluginMapperExtension(),
    ])


_user_extra_table = Table('user_extra', meta.metadata,
                         Column('id', types.UnicodeText, primary_key=True, default=_types.make_uuid),
                         Column('user_id', types.UnicodeText, ForeignKey('user.id')),
                         Column('key', types.UnicodeText),
                         Column('value', _types.JsonType),
                         )
vdm.sqlalchemy.make_table_stateful(_user_extra_table)

meta.mapper(UserExtra, _user_extra_table, properties={
    'user': orm.relation(user.User,
            ...
            order_by=[_user_extra_table.c.user_id, _user_extra_table.c.key],
            )
